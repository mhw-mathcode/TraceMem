...
def test_http_image_only_add_then_text_search_returns_original_part(tmp_path, monkeypatch):
    ...
    with TestClient(app_module.create_app(settings)) as client:
        add = client.post("/add", headers={"X-API-Key": "secret"}, json={
            "request_id": "r1", "user_id": "u1", "session_id": "s1",
            "messages": [{"role": "user", "content": parts}],
        })
        search = client.post("/search", headers={"X-API-Key": "secret"}, json={
            "query": "red bicycle", "user_id": "u1", "top_k": 5,
        })
        invalid = client.post("/add", headers={"X-API-Key": "secret"}, json={
